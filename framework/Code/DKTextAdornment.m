...
- (void)setLabel:(id)anySortOfText
{
    ...
    if ([anySortOfText isKindOfClass:[NSAttributedString class]]) {
        ...
        [[self textSubstitutor] setString:(NSString*)anySortOfText
                           withAttributes:attributes];
    } else {
        [[self textSubstitutor] setString:@""
                           withAttributes:[self defaultTextAttributes]];
    }
}
...
- (NSArray*)textPathsForObject:(id)object usedSize:(NSSize*)aSize
{
    ...
    if ([self layoutMode] == kDKTextLayoutAlongReversedPath ||
        ...
        NSArray* glyphs = [captureLM glyphPathsForContainer:container
                                                   usedSize:aSize];
...
- (DKStyle*)styleFromTextAttributes
{
    ...
    if (strokeColour && sw != 0.0) {
        DKStroke* stroke = [DKStroke strokeWithWidth:fabs(sw)
                                              colour:strokeColour];
...
- (void)scaleTextBy:(CGFloat)factor
{
    ...
    if (factor > 0.0 && factor != 1.0) {
        ...
        while (indx < [ms length]) {
            attr = (NSFont*)[ms attribute:NSFontAttributeName
                                  atIndex:indx
            ...
            if (attr) {
                ...
                attr = [[NSFontManager sharedFontManager] convertFont:attr
                                                               toSize:fontSize];

                [ms addAttribute:NSFontAttributeName
                           value:attr
                           range:range];
            }
...
{
    [self changeTextAttribute:NSBackgroundColorAttributeName
                      toValue:colour];
}
...
{
    [self changeTextAttribute:NSStrokeColorAttributeName
                      toValue:aColour];
}
    ...
    if (subs != mSubstitutor) {
        ...
        if (mSubstitutor)
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(masterStringChanged:)
                                                         name:kDKTextSubstitutorNewStringNotification
                                                       object:mSubstitutor];
    }
}
...
- (void)drawText:(NSTextStorage*)contents withObject:(id<DKRenderable>)obj withPath:(NSBezierPath*)path layoutManager:(NSLayoutManager*)lm
{
    ...
    if ([contents length] > 0) {
        ...
        NSRange fullRange = [lm glyphRangeForCharacterRange:NSMakeRange(0, [contents length])
        ...
        if (glyphRange.length > 0) {
            ...
            if (![self wrapsLines]) {
                frag = [lm lineFragmentUsedRectForGlyphAtIndex:0
...
- (void)drawKnockoutWithObject:(id<DKRenderable>)obj
{
    ...
    if ([self textKnockoutDistance] > 0.0 && !ghost) {
        ...
        if (textPath == nil) {
            ...
            [mTACache setObject:textPath
                         forKey:kDKTextAdornmentMaskPathCacheKey];
        }
...
- (NSSize)extraSpaceNeeded
{
    ...
    if (([self layoutMode] != kDKTextLayoutInBoundingRect) && [self enabled]) {
        ...
        NSRect tbr = [@"Dummy Text" boundingRectWithSize:textBoxSize
                                                 options:opts
                                              attributes:[self textAttributes]];
    ...
    @try
    {
        ...
        if ([self layoutMode] == kDKTextLayoutAtCentroid) {
            ...
            if ([self layoutMode] == kDKTextLayoutAlongReversedPath ||
                ...
                mLastLayoutFittedAllText = [path drawTextOnPath:str
                                                        yOffset:baseOffset
                                                  layoutManager:lm
                                                          cache:mTACache];
            } else {
...
- (void)registerActionNames
{
    ...
    [self setActionName:@"#kind# Text"
             forKeyPath:@"label"];
    [self setActionName:@"#kind# Identifier"
             forKeyPath:@"identifier"];
    [self setActionName:@"#kind# Font"
             forKeyPath:@"font"];
    [self setActionName:@"#kind# Paragraph Style"
             forKeyPath:@"paragraphStyle"];
    [self setActionName:@"#kind# Vertical Alignment"
             forKeyPath:@"verticalAlignment"];
    [self setActionName:@"#kind# Text Angle"
             forKeyPath:@"angle"];
    [self setActionName:@"#kind# Line Wrap"
             forKeyPath:@"wrapsLines"];
    [self setActionName:@"#kind# Text Layout"
             forKeyPath:@"layoutMode"];
    [self setActionName:@"#kind# Tracks Object Angle"
             forKeyPath:@"appliesObjectAngle"];
    [self setActionName:@"#kind# Text Inset"
             forKeyPath:@"flowedTextPathInset"];
    [self setActionName:@"#kind# Allow Horizontal Extension"
             forKeyPath:@"allowsTextToExtendHorizontally"];
    [self setActionName:@"#kind# Vertical Position"
             forKeyPath:@"verticalAlignmentProportion"];
    [self setActionName:@"#kind# Mask Amount"
             forKeyPath:@"textKnockoutDistance"];
    [self setActionName:@"#kind# Mask Colour"
             forKeyPath:@"textKnockoutColour"];
    [self setActionName:@"#kind# Mask Stroke Width"
             forKeyPath:@"textKnockoutStrokeWidth"];
    [self setActionName:@"#kind# Mask Stroke Colour"
             forKeyPath:@"textKnockoutStrokeColour"];
    [self setActionName:@"#kind# Capitalization"
             forKeyPath:@"capitalization"];
    [self setActionName:@"#kind# Placeholder String"
             forKeyPath:@"placeholderString"];
}
...
- (void)encodeWithCoder:(NSCoder*)coder
{
    ...
    [coder encodeObject:[self textSubstitutor]
                 forKey:@"DKTextAdornment_substitutor"];

    [coder encodeDouble:[self angle]
                 forKey:@"angle"];
    [coder encodeInteger:[self verticalAlignment]
                  forKey:@"valign"];
    [coder encodeInteger:[self layoutMode]
                  forKey:@"layout_mode"];
    [coder encodeBool:[self wrapsLines]
               forKey:@"wraps"];
    [coder encodeBool:[self appliesObjectAngle]
               forKey:@"objangle"];
    [coder encodeDouble:[self flowedTextPathInset]
                 forKey:@"DKTextAdornment_flowedTextInset"];
    [coder encodeBool:mAllowIndefiniteWidth
               forKey:@"DKTextAdornment_allowIndefWidth"];
    [coder encodeDouble:mVerticalPosition
                 forKey:@"DKTextAdornment_verticalPosition"];

    [coder encodeDouble:[self textKnockoutDistance]
                 forKey:@"DKTextAdornment_knockoutDistance"];
    [coder encodeObject:[self textKnockoutColour]
                 forKey:@"DKTextAdornment_knockoutColour"];
    [coder encodeDouble:[self textKnockoutStrokeWidth]
                 forKey:@"DKTextAdornment_knockoutStrokeWidth"];
    [coder encodeObject:[self textKnockoutColour]
                 forKey:@"DKTextAdornment_knockoutStrokeColour"];

    [coder encodeInteger:[self capitalization]
                  forKey:@"DKTextAdornment_capitalization"];
    [coder encodeObject:[self placeholderString]
