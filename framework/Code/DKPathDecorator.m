...
- (void)setUpCache
{
    ...
    if (m_pdf)
        ...
        [m_image drawAtPoint:NSZeroPoint
                    fromRect:NSZeroRect
...
- (void)registerActionNames
{
    ...
    [self setActionName:@"#kind# Image Scale"
             forKeyPath:@"scale"];
    [self setActionName:@"#kind# Image Spacing"
             forKeyPath:@"interval"];
    [self setActionName:@"#kind# Align To Path Curvature"
             forKeyPath:@"normalToPath"];
    [self setActionName:@"#kind# Image"
             forKeyPath:@"image"];
    [self setActionName:@"#kind# Leader Length"
             forKeyPath:@"leaderDistance"];
    [self setActionName:@"#kind# Ramp"
             forKeyPath:@"leadInAndOutLengthProportion"];
    [self setActionName:@"#kind# To Chain"
             forKeyPath:@"usesChainMethod"];
    [self setActionName:@"#kind# Alternating Offset"
             forKeyPath:@"lateralOffsetAlternates"];
    [self setActionName:@"#kind# Lateral Offset"
             forKeyPath:@"lateralOffset"];
    [self setActionName:@"#kind# Wobblyness"
             forKeyPath:@"wobblyness"];
    [self setActionName:@"#kind# Scale Randomness"
             forKeyPath:@"scaleRandomness"];
}
...
#pragma unused(userInfo)
    ...
    if (img != nil) {
        ...
        [tfm translateXBy:p.x + dx + wobblePoint.x
                      yBy:p.y + dy + wobblePoint.y];
        [tfm scaleXBy:[self scale] * leadScale * randScale
                  yBy:[self scale] * -1.0 * leadScale * randScale];

        if ([self normalToPath])
            [tfm rotateByRadians:-slope];

        [tfm translateXBy:-(iSize.width / 2)
            yBy:-(iSize.height / 2)];
...
#pragma unused(path)
    ...
    if ((lkn & 1) == pass) {
        NSBezierPath* linkPath = [NSBezierPath bezierPathWithStandardChainLinkFromPoint:pa
                                                                                toPoint:pb];
...
- (void)renderPath:(NSBezierPath*)path
{
    ...
    if ([self usesChainMethod]) {
        ...
        [path placeLinksOnPathWithLinkLength:[self interval]
                               factoryObject:self
                                    userInfo:&pass];
        ...
        [path placeObjectsOnPathAtInterval:[self interval]
                             factoryObject:self
                                  userInfo:NULL];
}
...
- (void)encodeWithCoder:(NSCoder*)coder
{
    ...
    [coder encodeDouble:[self scale]
                 forKey:@"scale"];
    [coder encodeDouble:[self interval]
                 forKey:@"interval"];
    [coder encodeDouble:[self leaderDistance]
                 forKey:@"leader"];
    [coder encodeDouble:[self leadInLength]
                 forKey:@"lead_in_length"];
    [coder encodeDouble:[self leadOutLength]
                 forKey:@"lead_out_length"];
    [coder encodeDouble:[self leadInAndOutLengthProportion]
                 forKey:@"lead_inout_proportion"];

    [coder encodeBool:[self normalToPath]
               forKey:@"normal"];
    [coder encodeBool:[self usesChainMethod]
               forKey:@"chainmeth"];

    [coder encodeDouble:mLateralOffset
                 forKey:@"DKPathDecorator_lateralOffset"];
    [coder encodeBool:mAlternateLateralOffsets
               forKey:@"DKPathDecorator_alternateLaterals"];
    [coder encodeDouble:mWobblyness
                 forKey:@"DKPathDecorator_wobblyness"];
    [coder encodeDouble:mScaleRandomness
