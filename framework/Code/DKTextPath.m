...
{
    return [DKStyle styleWithFillColour:nil
                           strokeColour:[NSColor clearColor]];
}
...
- (void)startEditingInView:(DKDrawingView*)view
{
    if (mEditorRef == nil) {
        ...
        mEditorRef = [view editText:[[self textAdornment] textForEditing]
                             inRect:br
                           delegate:self];
...
- (void)drawSelectedState
{
    if (![[self textAdornment] allTextWasFitted] && [DKTextShape showsTextOverflowIndicator]) {
        ...
        endPoint = [[self path] pointOnPathAtLength:[self length] - knobSize.width
                                              slope:&slope];

        NSAffineTransform* transform = [NSAffineTransform transform];
        [transform translateXBy:endPoint.x
                            yBy:endPoint.y];
        [transform rotateByRadians:slope];
        [transform scaleXBy:knobSize.width
                        yBy:-knobSize.height];
...
// the font menu may contain all the alignment commands - in which case we can leave these out
    ...
    [[convert addItemWithTitle:NSLocalizedString(@"Path", @"submenu item for convert to path")
...
- (BOOL)validateMenuItem:(NSMenuItem*)item
{
    ...
    if (action == @selector(changeFont:) || action == @selector(changeFontSize:) || action == @selector(changeAttributes:) || action == @selector(loosenKerning:) || action == @selector(tightenKerning:) || action == @selector(useStandardKerning:) || action == @selector(turnOffKerning:) || action == @selector(raiseBaseline:) || action == @selector(lowerBaseline:) || action == @selector(unscript:) || action == @selector(superscript:) || action == @selector(subscript:) || action == @selector(fitToText:) || action == @selector(convertToShapeGroup:) || action == @selector(convertToShape:) || action == @selector(convertToTextShape:) || action == @selector(convertToPath:) || action == @selector(editText:))
        return ![self locked];
