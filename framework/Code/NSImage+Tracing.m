...
- (NSArray*)vectorizeToGrayscale:(NSInteger)levels
{
    ...
    for (i = 0; i < levels; ++i) {
        rep = [[DKImageVectorRep alloc] initWithImageSize:[b8 size]
                                               pixelValue:i
                                                   levels:levels];
...
- (NSArray*)vectorizeToColourWithPrecision:(NSInteger)prec quantizationMethod:(DKColourQuantizationMethod)qm
{
    ...
    case kDKColourQuantizeUniform:
        quant = [[DKColourQuantizer alloc] initWithBitmapImageRep:b24
                                                       maxColours:levels
                                                       colourBits:prec];
        break;

    case kDKColourQuantizeOctree:
        quant = [[DKOctreeQuantizer alloc] initWithBitmapImageRep:b24
                                                       maxColours:levels
                                                       colourBits:prec];
        break;
    }
    ...
    for (i = 0; i < levels; ++i) {
        rep = [[DKImageVectorRep alloc] initWithImageSize:[b24 size]
                                               pixelValue:i
                                                   levels:levels];
        [bitplanes addObject:rep];
...
- (NSBitmapImageRep*)eightBitImageRep
{
    ...
    [self drawAtPoint:NSMakePoint(2, 2)
             fromRect:NSZeroRect
...
- (NSDictionary*)tracingParameters
{
    ...
    [dict setObject:[NSNumber numberWithInteger:mTraceParams->turdsize]
             forKey:kDKTracingParam_turdsize];
    [dict setObject:[NSNumber numberWithInteger:mTraceParams->turnpolicy]
             forKey:kDKTracingParam_turnpolicy];
    [dict setObject:[NSNumber numberWithDouble:mTraceParams->alphamax]
             forKey:kDKTracingParam_alphamax];
    [dict setObject:[NSNumber numberWithBool:mTraceParams->opticurve]
             forKey:kDKTracingParam_opticurve];
    [dict setObject:[NSNumber numberWithDouble:mTraceParams->opttolerance]
             forKey:kDKTracingParam_opttolerance];
