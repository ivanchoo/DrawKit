...
- (BOOL)setUpKVOForObserver:(id)object
{
    ...
    if (observables && [observables count] > 0)
        [self setUpObservables:observables
                   forObserver:object];
...
- (BOOL)tearDownKVOForObserver:(id)object
{
    ...
    if ((observables != nil) && [observables count] > 0)
        [self tearDownObservables:observables
                      forObserver:object];
...
- (NSString*)actionNameForKeyPath:(NSString*)keypath changeKind:(NSKeyValueChange)kind
{
    ...
    NSRange range = [an rangeOfString:@"#kind#"
...
{
    [GCObservableObject registerActionName:name
                                forKeyPath:keypath
                                  objClass:[self class]];
}
...
- (void)sendInitialValuesForAllPropertiesToObserver:(id)object context:(void*)context
{
    ...
    if (observables && [observables count] > 0) {
        ...
        while ((keyPath = [iter nextObject])) {
            ...
            [changeDict setObject:value
                           forKey:NSKeyValueChangeNewKey];
            [object observeValueForKeyPath:keyPath
                                  ofObject:self
                                    change:changeDict
                                   context:context];
        }
...
- (void)willChange:(NSKeyValueChange)change valuesAtIndexes:(NSIndexSet*)indexes forKey:(NSString*)key
{
    ...
    if (change == NSKeyValueChangeInsertion || change == NSKeyValueChangeRemoval || change == NSKeyValueChangeReplacement) {
...
#pragma unused(context)
    ...
    [changeDict setObject:keyPath
                   forKey:kDKObservableKeyPath];
