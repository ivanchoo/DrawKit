...
{
    [self drawControlBarWithKnobsFromPoint:a
                                    ofType:kDKControlPointKnobType
                                   toPoint:b
                                    ofType:kDKControlPointKnobType];
}
...
- (void)drawControlBarWithKnobsFromPoint:(NSPoint)a ofType:(DKKnobType)typeA toPoint:(NSPoint)b ofType:(DKKnobType)typeB
{
    ...
    [self drawControlBarFromPoint:a
                          toPoint:b];
    [self drawKnobAtPoint:a
                   ofType:typeA
                    angle:angle
                 userInfo:nil];
    [self drawKnobAtPoint:b
                   ofType:typeB
                    angle:angle
                 userInfo:nil];
}
...
- (void)drawRotationBarWithKnobsFromCentre:(NSPoint)centre toPoint:(NSPoint)p
{
    ...
    [self drawControlBarWithKnobsFromPoint:centre
                                    ofType:kDKCentreTargetKnobType
                                   toPoint:p
...
{
    return [self handleForType:knobType
                        colour:nil];
}

- (DKHandle*)handleForType:(DKKnobType)knobType colour:(NSColor*)colour
{
    return [DKHandle handleForType:knobType
                              size:[self actualHandleSize]
                            colour:colour];
}
...
- (void)encodeWithCoder:(NSCoder*)coder
{
    [coder encodeConditionalObject:[self owner]
                            forKey:@"DKKnob_ownerRef"];
    [coder encodeDouble:mScaleRatio
                 forKey:@"DKKnob_scaleRatio"];
    [coder encodeSize:[self controlKnobSize]
               forKey:@"DKKnob_controlKnobSize"];

    [coder encodeObject:[self controlKnobColour]
                 forKey:@"DKKnob_knobColour"];
    [coder encodeObject:[self rotationKnobColour]
                 forKey:@"DKKnob_rotationKnobColour"];
    [coder encodeObject:[self controlOnPathPointColour]
                 forKey:@"DKKnob_onPathPointColour"];
    [coder encodeObject:[self controlOffPathPointColour]
                 forKey:@"DKKnob_offPathPointColour"];
    [coder encodeObject:[self controlBarColour]
                 forKey:@"DKKnob_controlBarColour"];

    [coder encodeDouble:[self controlBarWidth]
                 forKey:@"DKKnob_controlBarWidth"];
}
...
{
    if (sKnobColour == nil)
        [DKKnob setControlKnobColour:[NSColor colorWithDeviceRed:0.5
                                                           green:0.9
                                                            blue:1.0
                                                           alpha:1.0]];
...
#pragma unused(userInfo)
    ...
    BOOL isRect = (knobType == kDKBoundingRectKnobType || knobType == kDKHotspotKnobType || knobType == kDKMoreTextIndicatorKnobType);
    NSBezierPath* path;
    NSRect boundsRect = [self controlKnobRectAtPoint:p
                                              ofType:knobType];
