...
- (void)setControlPoint:(NSPoint)p forPartcode:(NSInteger)pc
{
    ...
    [self elementAtIndex:elem
        associatedPoints:ap];
    ap[arrayIndexForPartcode(pc)] = p;

    [self setAssociatedPoints:ap
                      atIndex:elem];
}
...
{
    return [self partcodeHitByPoint:p
                          tolerance:t
...
- (NSInteger)partcodeHitByPoint:(NSPoint)p tolerance:(CGFloat)t prioritiseOnPathPoints:(BOOL)onpPriority
{
    return [self partcodeHitByPoint:p
                          tolerance:t
                startingFromElement:0
             prioritiseOnPathPoints:onpPriority];
}
...
- (NSInteger)partcodeHitByPoint:(NSPoint)p tolerance:(CGFloat)t startingFromElement:(NSInteger)startElement
{
    return [self partcodeHitByPoint:p
                          tolerance:t
                startingFromElement:startElement
...
- (NSPoint)referencePointForConstrainedPartcode:(NSInteger)pc
{
    ...
    NSBezierPathElement et = [self elementAtIndex:element
                                 associatedPoints:ap];
    ...
    if (element == 0 && indx == 0) {
        ...
        if (element + 1 < ec) {
            et = [self elementAtIndex:element + 1
                     associatedPoints:ap];
...
- (void)moveControlPointPartcode:(NSInteger)pc toPoint:(NSPoint)p colinear:(BOOL)colin coradial:(BOOL)corad constrainAngle:(BOOL)acon
{
    ...
    if (et == NSCurveToBezierPathElement) {
        ...
        case 1: // control point 2
            if (colin) {
                if ((element < (ec - 1)) && (following == NSCurveToBezierPathElement)) {
                    ...
                    [self setControlPoint:opp
                              forPartcode:partcodeForElement(next)];
                    ...
                    if (following == NSCurveToBezierPathElement) {
                        ...
                        [self setControlPoint:opp
                                  forPartcode:partcodeForElement(e2)];
                    }
                }
            }
            break;
        ...
        if (depth == 1 && closedLoop && (et == NSMoveToBezierPathElement) && colin) {
            ...
            if (following == NSCurveToBezierPathElement)
                [self moveControlPointPartcode:partcodeForElementControlPoint(ee, 2)
                                       toPoint:p
                                      colinear:colin
                                      coradial:corad
                                constrainAngle:acon];
            else
                [self moveControlPointPartcode:partcodeForElement(ee)
                                       toPoint:p
                                      colinear:colin
                                      coradial:corad
                                constrainAngle:acon];
...
{
    CGFloat t;
    NSInteger i, j, m, inselem = [self elementHitByPoint:p
                                               tolerance:tol
                                                  tValue:&t];
            ...
            if (i == inselem) {
                ...
                if (pe == NSCurveToBezierPathElement) {
                    ...
                    if (controlPointType == 1 || controlPointType == 3) {
                        ...
                        [newPath curveToPoint:b2[3]
                                controlPoint1:b2[1]
                                controlPoint2:b2[2]];
                    } else {
                        [newPath curveToPoint:b1[3]
                                controlPoint1:b1[1]
                                controlPoint2:b1[2]];
                        [newPath curveToPoint:b2[3]
                                controlPoint1:b2[1]
                    ...
                    if (controlPointType == 2 || controlPointType == 3) {
                        ...
                        [newPath curveToPoint:ip
                                controlPoint1:cpa
                                controlPoint2:cpb];
                        [newPath curveToPoint:ap[1]
                                controlPoint1:cpc
                                controlPoint2:cpd];
                    } else {
...
{
    return [self elementHitByPoint:p
                         tolerance:tol
                            tValue:t
                      nearestPoint:NULL];
}
...
// to count as a hit.
...
#if (USE_OMNI_METHODS)
    CGFloat tee;
    NSInteger elem = [self _segmentHitByPoint:p
                                     position:&tee
                                      padding:tol];
    ...
    if (NSPointInRect(p, bb)) {
        NSInteger elem = [self elementBoundsContainsPoint:p
                                                tolerance:tol];
        ...
        if (elem > 0) {
            ...
            NSPoint ap[3], lp[3];
            NSBezierPathElement etype = [self elementAtIndex:elem
                                            associatedPoints:ap];
            NSBezierPathElement pretype = [self elementAtIndex:elem - 1
                                              associatedPoints:lp];
            double tt = 0.0;
...
- (NSRect)boundingBoxForElement:(NSInteger)elementIndex
{
    ...
    NSBezierPathElement lm = [self elementAtIndex:elementIndex
                                 associatedPoints:ap];

    if (lm == NSMoveToBezierPathElement)
        return NSZeroRect;

    pm = [self elementAtIndex:elementIndex - 1
             associatedPoints:pp];
