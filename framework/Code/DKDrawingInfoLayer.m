...
- (void)drawInfoInRect:(NSRect)br
{
    ...
    r = [self layoutRectForDrawingInfoItem:kDKDrawingInfoDrawingNumber
                                    inRect:br];
    label = [self labelForDrawingInfoItem:kDKDrawingInfoDrawingNumber];
    lr = [self labelRectInRect:r
                      forLabel:label];
    ...
    [self drawString:infos
                inRect:r
        withAttributes:[self attributesForDrawingInfoItem:kDKDrawingInfoDrawingNumber]];
    [label drawInRect:NSOffsetRect(lr, 1.5, 0)];

    r = [self layoutRectForDrawingInfoItem:kDKDrawingInfoDrawingRevision
                                    inRect:br];
    ...
    [self drawString:infos
                inRect:r
        withAttributes:[self attributesForDrawingInfoItem:kDKDrawingInfoDrawingRevision]];
    [label drawInRect:NSOffsetRect(lr, 1.5, 0)];

    r = [self layoutRectForDrawingInfoItem:kDKDrawingInfoDraughter
                                    inRect:br];
    ...
    [self drawString:infos
                inRect:r
        withAttributes:[self attributesForDrawingInfoItem:kDKDrawingInfoDraughter]];
    [label drawInRect:NSOffsetRect(lr, 1.5, 0)];

    r = [self layoutRectForDrawingInfoItem:kDKDrawingInfoCreationDate
                                    inRect:br];
    ...
    infos = [dt descriptionWithCalendarFormat:@"%B %e, %Y"
                                     timeZone:nil
                                       locale:[[NSUserDefaults standardUserDefaults] dictionaryRepresentation]];
    //if ( _editingKey != kDKDrawingInfoCreationDate )
    [self drawString:infos
                inRect:r
        withAttributes:[self attributesForDrawingInfoItem:kDKDrawingInfoCreationDate]];
    [label drawInRect:NSOffsetRect(lr, 1.5, 0)];

    r = [self layoutRectForDrawingInfoItem:kDKDrawingInfoModificationHistory
                                    inRect:br];
...
- (NSAttributedString*)labelForDrawingInfoItem:(NSString*)key
{
    ...
    if ([key isEqualToString:kDKDrawingInfoDrawingNumber])
        s = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"dwg #:", @"drawing info number label")
                                            attributes:attr];
    else if ([key isEqualToString:kDKDrawingInfoDrawingRevision])
        s = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"rev:", @"drawing info revision label")
                                            attributes:attr];
    else if ([key isEqualToString:kDKDrawingInfoDraughter])
        s = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"by:", @"drawing info draughter label")
                                            attributes:attr];
    else if ([key isEqualToString:kDKDrawingInfoCreationDate])
        s = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"date:", @"drawing info creation date label")
                                            attributes:attr];
...
- (void)textViewDidChangeSelection:(NSNotification*)aNotification
{
    ...
    [di setObject:str
           forKey:m_editingKeyRef];
    NSRect r = [self layoutRectForDrawingInfoItem:m_editingKeyRef
                                           inRect:[self infoBoxRect]];
...
- (void)mouseDown:(NSEvent*)event inView:(NSView*)view
{
    ...
    if (![self locked]) {
        ...
        if (key != nil) {
            NSDictionary* di = [[self drawing] drawingInfo];
            NSRect fr = [self layoutRectForDrawingInfoItem:key
                                                    inRect:[self infoBoxRect]];
            NSRect lr = [self labelRectInRect:fr
                                     forLabel:[self labelForDrawingInfoItem:key]];
            NSTextStorage* str;
            ...
            [(DKDrawingView*)view editText:str
...
- (void)encodeWithCoder:(NSCoder*)coder
{
    ...
    [coder encodeInteger:[self placement]
                  forKey:@"infoBoxPlacement"];
    [coder encodeSize:[self size]
               forKey:@"infoBoxSize"];
    [coder encodeBool:[self drawsBorder]
               forKey:@"drawBorder"];
}
